...
function updateUIAfterLogin(userData) {
    ...
    // Обновляем текст ссылки профиля
    const profileLinkText = profileLink.querySelector('span');
    if (profileLinkText) {
        profileLinkText.textContent = userData.name.split(' ')[0];
    }
    
    // Показываем информацию на странице логина
...
function checkSavedLogin() {
    ...
    if (savedUser) {
        try {
            ...
            updateUIAfterLogin(userData);
            console.log('🔄 Восстановлен предыдущий вход:', userData.email);
        } catch (e) {
            console.warn('Ошибка восстановления сессии');
...
function logout() {
    ...
    // Обновляем интерфейс
    document.getElementById('login-nav-link').style.display = 'block';
    document.getElementById('profile-nav-link').style.display = 'none';
    
    // Скрываем информацию на странице логина
    document.querySelector('.g_id_signin').style.display = 'block';
    document.getElementById('user-info').style.display = 'none';
    
    // Если мы на странице профиля или логина - переходим на главную
    const activePage = document.querySelector('.page.active');
    if (activePage && (activePage.id === 'profile-page' || activePage.id === 'login-page')) {
        switchToPage('home');
    }
...
// Загружаем Google API и проверяем сохраненный вход при загрузке
document.addEventListener('DOMContentLoaded', function() {
    loadGoogleAPI();
    checkSavedLogin();
    
    // Обновляем обработчик переключения страниц
    const originalSwitchToPage = switchToPage;
    window.switchToPage = function(pageId, userId = null) {
        originalSwitchToPage(pageId, userId);
        
        // Если перешли на страницу логина - обновляем UI
        if (pageId === 'login') {
            const savedUser = localStorage.getItem('google_user');
            if (savedUser) {
                try {
